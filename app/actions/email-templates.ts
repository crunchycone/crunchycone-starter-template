...
export async function getAvailableTemplates(): Promise<{ success: boolean; templates?: EmailTemplate[]; message?: string }> {
  ...
  try {
    ...
    try {
      ...
      const templates: EmailTemplate[] = templateMetadata.map(template => {
        ...
        return {
          id: templateId,
          name: template.name.split('-').map(word => 
  ...
  try {
    ...
    try {
      ...
      if (!templateData) {
        ...
        const dataPath = join(process.cwd(), "templates", "email", language, templateName, "data-preview.json");
        try {
      ...
      const [language, templateName] = templateId.split('/');
      const rendered = await templateService.previewTemplate(templateName, templateData || {}, language);
