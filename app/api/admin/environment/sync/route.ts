...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAuthenticated) {
      return NextResponse.json(
        { error: "CrunchyCone authentication required" },
        { status: 401 }
      );
    }
    ...
    const syncResult = await syncVariable(key, envVar.localValue, envVar.crunchyconeValue, direction, isSecret);
...
async function syncVariable(key: string, localValue: string, remoteValue?: string, direction: string = "auto", isSecret: boolean = false): Promise<{message: string, action: string}> {
  ...
  try {
    if (direction === "pull") {
      ...
      if (hasLocal) {
        ...
        return { message: `Pushed local value to CrunchyCone as ${isSecret ? 'secret' : 'env var'}`, action: "local_to_remote" };
      } else {
    ...
    throw new Error(`Failed to sync variable: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

async function updateCrunchyConeVariable(key: string, value: string, isSecret: boolean = false): Promise<void> {
  const { execSync } = require("child_process");
